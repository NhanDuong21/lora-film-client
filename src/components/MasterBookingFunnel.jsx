...
const REGIONS = [
  { id: 'hcm', name: 'TP Hồ Chí Minh', theaters: ['Lora Nguyễn Du', 'Lora Thảo Điền'] },
  { id: 'hn', name: 'Hà Nội', theaters: ['Lora Royal City', 'Lora Bà Triệu'] },
  { id: 'dn', name: 'Đà Nẵng', theaters: ['Lora Đà Nẵng Sông Hàn'] },
  { id: 'kh', name: 'Khánh Hòa', theaters: ['Lora Nha Trang Trần Phú'] }
];
...
export default function MasterBookingFunnel({ onBackHome, onBookTicket }) {
  ...
  // Predefined mock showtimes categorized by formats
  const theaterFormats = [
                      ...
                      <div
                        ...
                        className={`p-3 rounded-xl border flex items-center gap-4 cursor-pointer transition-all ${
                          selectedMovie?.id === movie.id
                            ? 'bg-zinc-850/70 border-amber-500 shadow-md shadow-amber-500/5'
                            : 'bg-zinc-950 border-zinc-850 hover:border-zinc-700'
                        }`}
                      >
                        <div className="w-12 h-18 rounded-lg overflow-hidden shrink-0 border border-zinc-800">
                        ...
                        <div className="space-y-1.5 min-w-0">
                          ...
                          <p className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider truncate">{movie.genre}</p>
                          ...
                          <div className="space-y-3">
                            {theaterFormats.map((tf) => (
                              <div key={tf.format} className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 py-1.5 border-t border-zinc-900/80 last:border-b-0">
                                <span className="text-[9px] font-black uppercase tracking-wider text-zinc-550 min-w-[90px]">{tf.format}</span>
                                <div className="flex flex-wrap gap-2">
                                  {tf.times.map((time) => {
            ...
            <div className="bg-zinc-900 border border-zinc-850 rounded-2xl overflow-hidden shadow-2xl flex flex-col">
              ...
              <div className="aspect-[16/10] bg-zinc-950 relative overflow-hidden border-b border-zinc-800 flex items-center justify-center">
                {selectedMovie ? (
                  ...
                  <div className="flex flex-col items-center gap-2 text-zinc-600 py-10">
              ...
              <div className="p-5 space-y-4 flex-grow text-xs font-semibold text-zinc-350">
                ...
                {selectedShowtime && (
                  <div className="flex gap-2 p-2.5 bg-zinc-950 border border-zinc-850 rounded-lg text-[9px] font-bold text-zinc-500 leading-normal">
